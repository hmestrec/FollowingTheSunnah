  ...
  const fetchContent = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const wakingUpEarlyContent = data.find(record => record.id === "WakingUpEarly"); // Replace with your actual ID
        if (wakingUpEarlyContent) {
          setContent(wakingUpEarlyContent.content); // Set the content if found
        } else {
          console.error("Content not found for Waking Up Early.");
