  ...
  return (
    <div className={styles.marriageProfilesContainer}>
      <div className={styles.profileForm}>
        <h2 className={styles.heading}>{isEditing ? 'Edit Your Marriage Profile' : 'Create Your Marriage Profile'}</h2>
        <form onSubmit={handleSaveProfile}>
          <label className={styles.label}>
            User ID:
            <input type="text" value={userId} disabled className={styles.input} />
          </label>
          <label className={styles.label}>
            Name:
          ...
          <label className={styles.label}>
            Age:
            <input type="number" value={age} onChange={(e) => setAge(e.target.value)} required className={styles.input} />
          </label>
          <label className={styles.label}>
            Gender:
          ...
          <label className={styles.label}>
            Description:
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} required className={styles.textarea}></textarea>
          </label>
          <label className={styles.label}>
            Preferences:
            <textarea value={preferences} onChange={(e) => setPreferences(e.target.value)} required className={styles.textarea}></textarea>
          </label>
          <div className={styles.buttonGroup}>
            <button type="submit" className={styles.saveButton}>{isEditing ? 'Update Profile' : 'Create Profile'}</button>
            {isEditing && (
              <button type="button" onClick={handleDeleteProfile} className={styles.deleteButton}>
                Delete Profile
              </button>
            )}
          </div>
